...
1. **Convert Cobblestone** (slot `45`): consumes all cobblestone in inventory and converts to Points.
...
1. **Buy x1, x2, x4, x8, x16, x32, x64** (slots `19..25`): deduct Points and grant the item stack immediately.
2. **Back** (slot `49`): return to shop list at the previously viewed page.
3. Info panel (slot `4`) shows selected item summary.
4. Points balance indicator (slot `50`) shows current Points.
...
1. **Purchase** (slot `22`): deduct Points and start the registered service effect.
2. **Back** (slot `49`): return to shop list at the previously viewed page.
...
5. On success, the shop list refreshes and the player sees the new Points balance.
...
3. The executor runs only after Points deduction succeeds and is ledgered.
...
1. **Insufficient Points**: action is rejected before deduction; no side effects run.
...
4. **Effect execution failure**: exact deducted Points are refunded; player receives explicit failure reason; audit log captures the failure.
