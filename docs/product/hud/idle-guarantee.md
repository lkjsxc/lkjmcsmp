...
The idle message is a permanent background layer for players who have the action-bar HUD enabled. It is always present in enabled-player state and automatically becomes visible whenever no overlay is active.
...
4. **Always-On Idle Guarantee**: the action bar must never be blank for an online player with the HUD enabled. Idle is stored as a persistent source with `expiresAt = -1`.
...
4. If the HUD is disabled, plugin HUD state is dropped without sending a blank action bar.
